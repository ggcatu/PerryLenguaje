...
class raiz : public ArbolSintactico {
	public:
		...
		raiz() {}
		raiz(ArbolSintactico * i, bool m) : includes(i), module(m) {}
...
class tipo : public ArbolSintactico {
	public:
		ArbolSintactico * tipo;
		ArbolSintactico * id;
		tipo(ArbolSintactico * i, ArbolSintactico * t) : id(i), tipo(t) {}
		virtual void imprimir(int tab){
			cout << "TYPE" << endl;
			for (int j = 0; j < tab+1; j++) cout << "	";
			cout << "IDENTIFICADOR:" << endl;
...
class programa : public ArbolSintactico {
	public:
		ArbolSintactico * estructura;
		ArbolSintactico * type;
		ArbolSintactico * funcion;
		ArbolSintactico * sec;
		ArbolSintactico * programa;
		programa(ArbolSintactico * p, ArbolSintactico * s) : programa(p), estructura(s) {}
		programa(ArbolSintactico * p, ArbolSintactico * s) : programa(p), type(s) {}
		programa(ArbolSintactico * p, ArbolSintactico * s) : programa(p), funcion(s) {}
		programa(ArbolSintactico * p, ArbolSintactico * s) : programa(p), sec(s) {}
		virtual void imprimir(int tab){
			if (programa != NULL) {
				programa -> imprimir(tab);
			}
			if (estructura != NULL){
				estructura -> imprimir(tab);
			} 
			if (type != NULL){
				type -> imprimir(tab);
			}
			if (funcion != NULL){
				funcion -> imprimir(tab);
			}
			if (sec != NULL){
				cout << "MAIN:" << endl;
				sec -> imprimir(tab+1);
...
class instruccion : public ArbolSintactico {
	public:
		...
		instruccion(ArbolSintactico * is, ArbolSintactico * i) : instrucciones(is), instruccion(i) {}
		instruccion(ArbolSintactico * i) : instruccion(i) {}
		virtual void imprimir(int tab){
			if (instrucciones != NULL){
				instrucciones -> imprimir(tab);
			}
			if (instruccion != NULL){
				instruccion -> imprimir(tab);
			}
		}
};
...
class exp_aritmetica : public ArbolSintactico {
	enum inst {SUMA, RESTA, MULT, DIV, MOD, RESTA, POW, PARENTESIS, CORCHETE, LLAVE};
	public:
		ArbolSintactico * der;
		ArbolSintactico * izq;
		inst instruccion;
		exp_aritmetica(ArbolSintactico * d, ArbolSintactico * i, int is) : der(d), izq(i), instruccion(static_cast<inst>(is)) {}
		exp_aritmetica(ArbolSintactico * d, int is) : der(d), izq(i), instruccion(static_cast<inst>(is)) {}
		virtual void imprimir(int tab){
			if (izq != NULL){
				izq -> imprimir(tab+1);
			}
...
class exp_booleana : public ArbolSintactico {
	...
	public:
		...
		exp_aritmetica(ArbolSintactico * d, int is) : der(d), izq(i), instruccion(static_cast<inst>(is)), ArbolSintactico(BOOLEANOS) {}
